...
//				} 
				...
				$userSql = $this->dbConnector->select()->from(array("u" =>$this->config->getOptionValue("users_table", "db_tables")))
						->join(array("g"=>$this->config->getOptionValue("groups_table", "db_tables")), "g.id_group = u.id_group" , "left", array("*", "gname" => "name"))
				...
				if (!($userResult)){
					...
					if (htmlentities($_POST["login_passwd"],ENT_QUOTES) == $userResult->password){
