...
//		Základní select
		$sqlSelect = $this->getDb()->select()->where("usr.".self::COLUM_DELETE." = ".(int)false);
...
//				->where("item.".Rights::RIGHTS_GROUPS_TABLE_PREFIX.$userNameGroup." LIKE \"r__\"")
		...
		$count = $this->getDb()->fetchAssoc($sqlCount->from(array("usr"=>$this->getModule()->getDbTable()), array("count"=>"COUNT(*)")), true);
...
//		Základní select
		$sqlSelect = $sqlSelect->from(array("usr"=>$this->getModule()->getDbTable()))
...
//		Mazání záznamu
		if(isset($_POST[self::FORM_PREFIX.self::FORM_DELETE]) AND is_numeric($_POST[self::FORM_PREFIX.self::FORM_ID])){
			...
			// Načtení jména uživatele z db
			$sqlSelect = $this->getDb()->select()->from(array("usr"=>$this->getModule()->getDbTable()))
												 ->where("usr.".self::COLUM_DELETE." = ".(int)false)
												 ->where("usr.".self::COLUM_ID." = ".$idUser);
			
			$user = $this->getDb()->fetchAssoc($sqlSelect, true);
			
//			Uložení změny
			$change = $this->eplugin()->changes()->createChange("Smazán uživatel \"".$user[self::COLUM_SURNAME].", ".$user[self::COLUM_NAME]." - ".$user[self::COLUM_USERNAME]."\"", $idUser);
...
//		Link pro editaci
		...
		//		Načtení provedených změn
		$this->getModel()->changes = $this->eplugin()->changes()->getChanges($idUser);
	}
		...
		if(isset($_POST[self::FORM_PREFIX.self::FORM_SEND])){
			...
			if($this->checkEditUserParams()){
				...
				if(!$this->getDb()->query($sqlUpd)){
					...
					$change = $this->eplugin()->changes()->createChange("Upravení uživatele \"".$user[self::COLUM_SURNAME].", ".$user[self::COLUM_NAME]." - ".$user[self::COLUM_USERNAME]."\"", $idUser);
...
//			Jsou li vyplněny všechny povinné údaje
			...
			if($this->checkNewUserParams()){	
				...
				$change = $this->eplugin()->changes()->createChange("Přidání uživatele \"".$surname.", ".$name." - ".$username."\"", $this->getDb()->getLastInsertedId());
