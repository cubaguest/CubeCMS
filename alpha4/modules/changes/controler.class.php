...
//		Základní select
		...
		if(isset($_POST[self::SEARCH_PREFIX.self::SEARCH_SEND])){
			...
			$_POST[self::SEARCH_PREFIX.self::SEARCH_USERNAME]!= null ?	$this->searchArray[self::SEARCH_SURNAME] = htmlspecialchars($_POST[self::SEARCH_PREFIX.self::SEARCH_USERNAME], ENT_QUOTES) : null;
...
//				->where("item.".Rights::RIGHTS_GROUPS_TABLE_PREFIX.$userNameGroup." LIKE \"r__\"")
		...
		$sqlCount = $sqlSelect;
//		echo $sqlSelect->from(array("changes"=>$this->getModule()->getDbTable()), array("count"=>"COUNT(*)"))
//					  ->join(array("user"=>$this->getModule()->getDbTable(2)), "user.".self::COLUM_ID_USER." = changes.".self::COLUM_ID_USER, "INNER", null);
