...
class Template_JsPlugin extends Template {
   ...
   public function addTplFile($name) {
       array_push($this->templateFiles, self::getFileDir($name, $this->jsPlugin()->getName()).$name);
   }
   ...
   public function addJsFile($jsfile, $engine = false) {
      ...
      if(eregi('http://[a-zA-Z_.]+', $jsfile)){
         ...
         $filePath = parent::getFileDir($jsfile, $this->jsPlugin()->getName());
         if($filePath != null){
            self::addJS($filePath.$jsfile);
         }
      }
   ...
   public function addCssFile($cssfile, $engine = false) {
      if(eregi('http://[a-zA-Z_.]+', $cssfile)){
         ...
         $filePath = self::getFileDir($cssfile, $this->jsPlugin()->getName());
         if($filePath!=null){
            self::addCss($filePath.$cssfile);
         }
      }
    ...
    */
   public static function getFileDir($file, $pluginName = null, $realpath = true) {
      ...
      if(file_exists($faceDir.$file)) {
          if($realpath){
             ...
             return Url_Request::getBaseWebDir().self::FACES_DIR.URL_SEPARATOR.self::$face.URL_SEPARATOR
         ...
         if($realpath){
            ...
            return Url_Request::getBaseWebDir().JsPlugin::JSPLUGINS_BASE_DIR.URL_SEPARATOR.$pluginName.URL_SEPARATOR;
            return null;
         }
      } else {
