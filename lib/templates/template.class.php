...
class Template {
   ...
   public function v($name) {
      ...
      if (isset ($this->privateVars[$name])){
         return $this->privateVars[$name]->get();
//         return $this->privateVars[$name];
   ...
   public function setPublicVar($var) {
   ...
   public function addTplFile($name, $engine = false) {
      array_push($this->templateFiles, $this->getFileDir($name, self::TEMPLATES_DIR, $engine).$name);
   }
    ...
    */
   public function insertTpl($name, $engine = false) {
      include_once ($this->getFileDir($name, self::TEMPLATES_DIR, $engine)).$name;
   }
   ...
   public function addTplObj($name, Template $obj) {
      $this->templateObjects[$name] = clone $obj;
   }

   // metoda vyrenderuje zadanou šablonu
   public function renderTplObj($name) {
    ...
    */
     ...
     */
   protected function getFileDir($file, $type = self::TEMPLATES_DIR, $engine = false) {
      $moduleDir = null;
      if(!$engine AND $this->module() != null){
         $moduleDir = $this->module()->getDir()->getMainDir(false).DIRECTORY_SEPARATOR;
      }
      ...
      if(file_exists(AppCore::MAIN_ENGINE_PATH.self::FACES_DIR.DIRECTORY_SEPARATOR
            .self::$face.$moduleDir.DIRECTORY_SEPARATOR.$type.DIRECTORY_SEPARATOR.$file)){
         return AppCore::MAIN_ENGINE_PATH.self::FACES_DIR.DIRECTORY_SEPARATOR
         .self::$face.$moduleDir.DIRECTORY_SEPARATOR.$type.DIRECTORY_SEPARATOR;
      }
      ...
      // použijeme soubor přímo s enginu
         ...
         return AppCore::MAIN_ENGINE_PATH.$type.DIRECTORY_SEPARATOR;
      }
      return _('NOT EXIST OBJECT!');
   }
