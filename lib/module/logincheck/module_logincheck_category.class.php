...
class Module_LoginCheck_Category extends Category_Core {
   /**
   ...
   private function createCatObj() {
      ...
      $category->{Model_Category::COLUMN_MODULE} = 'logincheck';
      $category->{Model_Category::COLUMN_URLKEY} = 'logincheck.php';
      $category->{Model_Category::COLUMN_CAT_LABEL} = $this->tr('Kontrola přihlášení');
      $category->{Model_Category::COLUMN_INDIVIDUAL_PANELS} = false;
      $category->{Model_Category::COLUMN_DESCRIPTION} = $this->tr('Kontrola přihlášení');
