...
class Module_ShopFeedZbozi_Category extends Category_Core {
   /**
   ...
   private function createCatObj() {
      ...
      $category->{Model_Category::COLUMN_MODULE} = 'shopfeedzbozi';
      $category->{Model_Category::COLUMN_CAT_LABEL} = $this->tr('XML feed zboží pro Zbozi.cz');
      $category->{Model_Category::COLUMN_INDIVIDUAL_PANELS} = false;
      $category->{Model_Category::COLUMN_DESCRIPTION} = $this->tr('XML feed zboží pro Zbozi.cz');
