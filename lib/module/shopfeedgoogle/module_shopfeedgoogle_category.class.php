...
class Module_ShopFeedGoogle_Category extends Category_Core {
   /**
   ...
   private function createCatObj() {
      ...
      $category->{Model_Category::COLUMN_MODULE} = 'shopfeedgoogle';
      $category->{Model_Category::COLUMN_CAT_LABEL} = $this->tr('XML feed zboží pro Google');
      $category->{Model_Category::COLUMN_INDIVIDUAL_PANELS} = false;
      $category->{Model_Category::COLUMN_DESCRIPTION} = $this->tr('XML feed zboží pro Google');
