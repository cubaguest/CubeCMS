...
class Module_Cookieinfo_Category extends Category_Core {
   /**
   ...
   private function createCatObj() {
      ...
      // Je třeba více?
      $category->{Model_Category::COLUMN_CAT_ID} = 32800;
      $category->{Model_Rights::COLUMN_RIGHT} = 'r--';
      $category->{Model_Category::COLUMN_MODULE} = 'cookieinfo';
      $category->{Model_Category::COLUMN_CAT_LABEL} = $this->tr('Prohlášení o použití cookies');
      $category->{Model_Category::COLUMN_INDIVIDUAL_PANELS} = false;
      $category->{Model_Category::COLUMN_DESCRIPTION} = $this->tr('Prohlášení o použití cookies');
