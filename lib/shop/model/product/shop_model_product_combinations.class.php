 ...
 */
class Shop_Model_Product_Combinations extends Model_ORM {
   ...
   protected function _initTable()
   {
      ...
      $this->addRelatioOneToMany(self::COLUMN_ID, 'Shop_Model_Product_CombinationHasVariant', Shop_Model_Product_CombinationHasVariant::COLUMN_ID_COMBINATION);
   }
   ...
   public static function generateDefaultCombination($idProduct)
   {
      ...
      $defaultVariants = $modelVariants->where(
         Shop_Model_Product_Variants::COLUMN_IS_DEFAULT." = 1 AND ".Shop_Model_Product_Variants::COLUMN_ID_PRODUCT." = :idp",
         array('idp' => $idProduct))->records(PDO::FETCH_OBJ);

      $default = array();
      foreach ($defaultVariants as $variant) {
         $default[] = $variant->{Shop_Model_Product_Variants::COLUMN_ID};
      }

//      $storedVariants = $modelComb
//         ->join(Shop_Model_Product_Combinations::COLUMN_ID, "Shop_Model_Product_CombinationHasVariant",
//         Shop_Model_Product_CombinationHasVariant::COLUMN_ID_COMBINATION)
...
//         if(in_array($var->{Shop_Model_Product_CombinationHasVariant::COLUMN_ID_VARIANT}, $default)){
//            if(!isset( $varCounter[$var->{Shop_Model_Product_Combinations::COLUMN_ID}])){
//               $varCounter[$var->{Shop_Model_Product_Combinations::COLUMN_ID}] = 1;
//            } else {
//               $varCounter[$var->{Shop_Model_Product_Combinations::COLUMN_ID}]++;
//            }
//            if($varCounter[$var->{Shop_Model_Product_Combinations::COLUMN_ID}] == $defaultCount){
//               $idCombination = $var->{Shop_Model_Product_Combinations::COLUMN_ID};
//               break;
//            }
//         }
//      }
      ...
      $modelComb
         ->where(Shop_Model_Product_Combinations::COLUMN_ID_PRODUCT." = :idp",
         array('idp' => $idProduct))
         ->update(array(Shop_Model_Product_Combinations::COLUMN_IS_DEFAULT => false));

      // nastav novou výchozí
      $combination = self::getCombinationByVariants($idProduct, $default);
      if($combination){
         // pokud je kombinace
         $comb = $modelComb->record($combination->{Shop_Model_Product_Combinations::COLUMN_ID});
         if($comb){
            $comb->{Shop_Model_Product_Combinations::COLUMN_IS_DEFAULT} = true;
            $comb->save();
         }
      } else {
         ...
         if(count($combs) > 0){
            $combs[0]->{Shop_Model_Product_Combinations::COLUMN_IS_DEFAULT} = true;
            $combs[0]->save();
         }
      }
   ...
   public static function getCombinationByVariants($idProduct, $variantsArray)
   {
      ...
      $comb = $model
         ->columns(array(
            '*',
            'num_variants' => 'COUNT(var_table.'.Shop_Model_Product_Variants::COLUMN_ID.')'
         ))
         ->join(self::COLUMN_ID,
            array( 'comb_var' => 'Shop_Model_Product_CombinationHasVariant'),
            Shop_Model_Product_CombinationHasVariant::COLUMN_ID_COMBINATION
         )
         ->join(array('comb_var' => Shop_Model_Product_CombinationHasVariant::COLUMN_ID_VARIANT),
            array('var_table' => 'Shop_Model_Product_Variants'),
            Shop_Model_Product_Variants::COLUMN_ID
         )
         ->where(self::COLUMN_ID_PRODUCT." = :idp AND comb_var.".Shop_Model_Product_Variants::COLUMN_ID." IN (".implode(',', array_keys($varForSQL)).")",
   ...
   public static function getDefaultCombination($idProduct)
   {
      ...
      return $model->columns(array(
         '*',
         'variant_ids' => 'GROUP_CONCAT(comb_var.'.Shop_Model_Product_Variants::COLUMN_ID.' SEPARATOR ";")',
         'price_add' => 'SUM(var_t.'.Shop_Model_Product_Variants::COLUMN_PRICE_ADD.')',
         'weight_add' => 'SUM(var_t.'.Shop_Model_Product_Variants::COLUMN_WEIGHT_ADD.')',
         ))
         ->join(self::COLUMN_ID,
            array( 'comb_var' => "Shop_Model_Product_CombinationHasVariant"),
            Shop_Model_Product_CombinationHasVariant::COLUMN_ID_COMBINATION)
         ->join(array('comb_var' => Shop_Model_Product_Variants::COLUMN_ID),
            array( 'var_t' => "Shop_Model_Product_Variants"),
            Shop_Model_Product_Variants::COLUMN_ID)
...
//      Debug::log($model->getSQLQuery());
   ...
   public static function generateCombinations($groupsVariants, $defaultVariants, $idProduct, $qty, $updatePrice = false, $onlyUpdate = false)
   {
      ...
      foreach ($variants as $variant) {
         $modelCombAttr = new Shop_Model_Product_CombinationHasVariant();
         $modelComb = new Shop_Model_Product_Combinations();
         ...
         if(!$combination){
            ...
            $combination->{Shop_Model_Product_Combinations::COLUMN_ID_PRODUCT} = $idProduct;
            $combination->{Shop_Model_Product_Combinations::COLUMN_QTY} = $qty;
         } else {
            $combination = $modelComb->record($combination->{Shop_Model_Product_Combinations::COLUMN_ID});
         }

         if(($updatePrice || $combination->isNew()) ){
            $combination->{self::COLUMN_PRICE} = Shop_Model_Product_Variants::getVariantsPrice($variant);
         }

         // create combination
         $combination->{Shop_Model_Product_Combinations::COLUMN_IS_DEFAULT} = false;

         // set default combination
         $compareDefaults = array_diff( $variant, $defaultVariants );
         if( empty( $compareDefaults ) ){
            $combination->{Shop_Model_Product_Combinations::COLUMN_IS_DEFAULT} = true;
         }
         $combination->save();

         // connect with attributes
         // delete prev attributes
         $modelCombAttr->where(Shop_Model_Product_CombinationHasVariant::COLUMN_ID_COMBINATION." = :idc",
            array('idc' => $combination->getPK()))->delete();
         foreach ($variant as $attr) {
            $comAttr = $modelCombAttr->newRecord();
            $comAttr->{Shop_Model_Product_CombinationHasVariant::COLUMN_ID_VARIANT} = $attr;
            $comAttr->{Shop_Model_Product_CombinationHasVariant::COLUMN_ID_COMBINATION} = $combination->getPK();
            $comAttr->save();
         }
      }
      ...
      $forDelete = $modelComb
         ->columns(array(
            Shop_Model_Product_Combinations::COLUMN_ID,
            'num_variants' => 'COUNT(vari.'.Shop_Model_Product_CombinationHasVariant::COLUMN_ID_VARIANT.')'
         ))
         ->join(Shop_Model_Product_Combinations::COLUMN_ID, array('comb_var' => "Shop_Model_Product_CombinationHasVariant"),
            Shop_Model_Product_CombinationHasVariant::COLUMN_ID_COMBINATION)
         ->join(
            array('comb_var' => Shop_Model_Product_CombinationHasVariant::COLUMN_ID_VARIANT),
            array('vari' => "Shop_Model_Product_Variants"),
            Shop_Model_Product_Variants::COLUMN_ID,
            array('*'), Model_ORM::JOIN_LEFT, ' AND vari.'.Shop_Model_Product_Variants::COLUMN_ID_PRODUCT." = :idvp",
            array('idvp' => $idProduct)
         )
         ->where(Shop_Model_Product_Combinations::COLUMN_ID_PRODUCT.' = :idp', array('idp' => $idProduct) )
         ->groupBy(Shop_Model_Product_Combinations::COLUMN_ID)
   ...
   public function prepareForProductCombinations($idProduct)
   {
      $this // setup model
         ->columns(array(
         Shop_Model_Product_Combinations::COLUMN_ID,
         Shop_Model_Product_Combinations::COLUMN_QTY,
         Shop_Model_Product_Combinations::COLUMN_PRICE,
         Shop_Model_Product_Combinations::COLUMN_IS_DEFAULT,
         'comb_name' =>
...
//         'SUM(vari.'.Shop_Model_Product_Variants::COLUMN_PRICE_ADD.')',
         'weight' =>
         'SUM(vari.'.Shop_Model_Product_Variants::COLUMN_WEIGHT_ADD.')',
         'comb_codes_json' =>
         "CONCAT('{', GROUP_CONCAT( '\"', attr.".Shop_Model_Attributes::COLUMN_ID_GROUP.", '\"', \" : \","
            ." '\"', vari.".Shop_Model_Product_Variants::COLUMN_CODE_ADD.", '\"' ORDER BY ".Shop_Model_Attributes::COLUMN_ORDER." SEPARATOR ', '),'}')",
         'comb_variant_ids' =>
         "GROUP_CONCAT( vari.".Shop_Model_Product_Variants::COLUMN_ID." SEPARATOR ',')",
      ))
      ->join(
            Shop_Model_Product_Combinations::COLUMN_ID,
            array( 'comb_var' => "Shop_Model_Product_CombinationHasVariant"),
      ...
      ->join(
            array('vari' => Shop_Model_Product_Variants::COLUMN_ID_ATTR),
      ...
      ->join(
         ...
         ->where(Shop_Model_Product_Combinations::COLUMN_ID_PRODUCT." = :idp", array('idp' => $idProduct))
         ->groupBy(array(Shop_Model_Product_Combinations::COLUMN_ID));
