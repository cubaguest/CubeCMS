 ...
 */
   ...
   public function addTplFile($name, $type = null) {
         if($type === null){
            array_push($this->templateFiles, self::getFileDir($name, self::TEMPLATES_DIR, $this->category->getModule()->getName(),true).$name);
         } else if($type === true){
   ...
   public static function getFileDir($file, $dir = self::TEMPLATES_DIR, $moduleName = null, $realpath = false, $withFile = false) {
      ...
      if(file_exists($faceDir.$file)) {
          if($realpath){
             ...
             $ret = Url_Request::getBaseWebDir().self::FACES_DIR.URL_SEPARATOR.self::$face.URL_SEPARATOR.AppCore::MODULES_DIR.URL_SEPARATOR.$moduleName.URL_SEPARATOR;
          }
