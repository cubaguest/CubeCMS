 ...
 */
    ...
    */
   public function saveRight($right, $idGroup, $idCat) {
      $dbc = new Db_PDO();
      $dbst = $dbc->query("SELECT COUNT(*) FROM ".self::getRightsTable()." AS rights"
      ...
      if ($dbst) {
         ...
         // update
            $dbst = $dbc->prepare("UPDATE ".self::getRightsTable(). " SET"
                ." `".self::COLUMN_RIGHT."` = :rights"
                ." WHERE (".self::COLUMN_ID_CATEGORY." = :idcat AND ".self::COLUMN_ID_GROUP." = :idgrp)");

         } else {
         // insert
            $dbst = $dbc->prepare("INSERT INTO ".self::getRightsTable()
