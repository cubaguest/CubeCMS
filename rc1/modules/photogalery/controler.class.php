...
//			Načtení fotek galerie
			$sqlSelect = $this->getDb()->select()->from(array('photos'=>$this->getModule()->getDbTable(1)), array(self::COLUM_PHOTOS_LABEL_IMAG => "IFNULL(photos.".self::COLUM_PHOTOS_LABEL_LANG_PREFIX.Locale::getLang().",
						...
						->order('photos.'.self::COLUM_PHOTOS_TIME)
						->order('photos.'.self::COLUM_PHOTOS_LABEL_LANG_PREFIX.Locale::getDefaultLang());
...
//			načtení fotky z db
			$sqlSelect = $this->getDb()->select()->from(array('photo' => $this->getModule()->getDbTable()),
				...
				->order('photo.'.self::COLUM_PHOTOS_LABEL_LANG_PREFIX.Locale::getDefaultLang());
...
//		}
	...
	private function deletePhotos($id, $allPhotosFromGalery = false ) {
		if(!$allPhotosFromGalery){
			...
			if($files->exist($this->getModule()->getDir()->getDataDir().self::IMAGES_THUMBNAILS_DIR, $photoDetail->{self::COLUM_PHOTOS_FILE}) AND
				!$files->deleteFile($this->getModule()->getDir()->getDataDir().self::IMAGES_THUMBNAILS_DIR, $photoDetail->{self::COLUM_PHOTOS_FILE})){
				$allFilesDeleted = false;
			}
			if($files->exist($this->getModule()->getDir()->getDataDir().self::IMAGES_MEDIUM_THUMBNAILS_DIR, $photoDetail->{self::COLUM_PHOTOS_FILE}) AND
				!$files->deleteFile($this->getModule()->getDir()->getDataDir().self::IMAGES_MEDIUM_THUMBNAILS_DIR, $photoDetail->{self::COLUM_PHOTOS_FILE})){
				$allFilesDeleted = false;
			}
			if($files->exist($this->getModule()->getDir()->getDataDir(), $photoDetail->{self::COLUM_PHOTOS_FILE}) AND
				!$files->deleteFile($this->getModule()->getDir()->getDataDir(), $photoDetail->{self::COLUM_PHOTOS_FILE})){
				$allFilesDeleted = false;
			}
