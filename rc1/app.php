...
//		Vypsání chyb na výstup
	...
	private function _initDbConnector() {
		...
		$this->dbConnector = Db::factory(self::sysConfig()->getOptionValue("dbhandler", self::MAIN_CONFIG_DB_SECTION),
...
//		require_once ('.' . DIRECTORY_SEPARATOR . 'lib' . DIRECTORY_SEPARATOR. 'template.class.php');
	...
	private function coreAuth() {
		$this->auth = new Auth($this->dbConnector, $this->userErrors);
	}
...
//		$this->_appMainDir = $appMainDir;
	...
	public function createMainMenu() {
		...
		if(class_exists("Menu")){
			$menu = new Menu($this->dbConnector, $this->auth);
...
//					textdomain(self::GETTEXT_DEFAULT_DOMAIN); 
					...
					$controller = new $controllerClassName($module, $this->dbConnector, $action, $moduleRights, $this->messages, $this->userErrors, $article);
...
//		$this->category = new Category($this->dbConnector, self::sysConfig(), $this->auth);
		Category::factory($this->dbConnector, $this->auth);
