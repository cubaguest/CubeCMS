...
class ShopProductGeneral_View extends Shop_Product_View {
   ...
   public function mainView()
   {
      ...
      if ($this->category()->getRights()->isWritable()) {
         ...
         $toolAdd = new Template_Toolbox2_Tool_PostRedirect('add_product', $this->tr('Přidat produkt'), $this->link()->route('add'));
...
//         }
   ...
   public function detailView()
   {
      ...
      if ($this->category()->getRights()->isWritable()) {
         ...
         $toolEdit = new Template_Toolbox2_Tool_PostRedirect('edit_product', $this->tr('Upravit produkt'), $this->link()->route('edit'));
         $toolEdit->setIcon('page_edit.png')->setTitle($this->tr("Upravit vybrané zboží"));
         $toolbox->addTool($toolEdit);

         $toolEditVariants = new Template_Toolbox2_Tool_PostRedirect('edit_product_variants', $this->tr('Upravit varianty'), $this->link()->route('editVariants'));
         ...
         $toolEditImages = new Template_Toolbox2_Tool_PostRedirect('edit_product_images', $this->tr('Upravit obrázky'), $this->link()->route('editImages'));
         ...
         $toolEditParams = new Template_Toolbox2_Tool_PostRedirect('edit_product_params', $this->tr('Upravit parametry'), $this->link()->route('editParams'));
