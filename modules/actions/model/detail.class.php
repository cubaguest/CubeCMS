...
class Actions_Model_Detail extends Model_PDO {
    ...
    */
   public function saveAction($name, $subname, $author, $text, $urlkey, DateTime $dateFrom, DateTime $dateTo,
...
//              ." LIMIT 0, 1");
   ...
   public function getCurrentAction($idCat) {
      ...
      $dbst = $dbc->prepare("SELECT * FROM ".Db_PDO::table(self::DB_TABLE)
              ...
              ." AND ((".self::COLUMN_DATE_START." < :timeNow AND ".self::COLUMN_DATE_STOP." > :timeNow)"
              ." OR (".self::COLUMN_DATE_START." > :timeNow))"
              ." LIMIT 0, 1");
      $dbst->bindValue(':timeNow', time(), PDO::PARAM_INT);
      $dbst->bindParam(':idcat', $idCat, PDO::PARAM_INT);
      $dbst->execute();
