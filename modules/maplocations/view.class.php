...
class MapLocations_View extends View {
   ...
   protected function createMapToolbox() {
      if($this->category()->getRights()->isWritable()) {
         ...
         $toolEdit->setIcon('page_edit.png')->setTitle($this->tr('Upravit místa na mapě'));
   ...
   protected function createListToolbox() {
      if($this->rights()->isWritable()) {
         ...
         $this->link()->route('add'));
         $toolAdd->setIcon('page_add.png')->setTitle($this->tr('Přidat nové místo'));
         ...
         $toolEdit->setIcon('page_edit.png')->setTitle($this->tr('Upravit vybrané místo'));
         $this->toolboxItem->addTool($toolEdit);
         
         $toolDelete = new Template_Toolbox2_Tool_Form($this->formDel);
         $toolDelete->setIcon('page_delete.png')->setTitle($this->tr('Smazat vybrané místo'))->setConfirmMeassage($this->tr('Opravdu smazat tuto položku?'));
