<?php
class ContactsSearch extends Search {
	public function runSearch() {

      $sqlSearch = $this->getDb()->select()->table($this->getModule()->getDbTable())
      ->colums(array(
            parent::RESULT_INDEX_RELEVANCE => 'MATCH ('.ReferenceModel::COLUMN_REFERENCE_NAME.'_'.Locales::getLang()
            .') AGAINST (\''.$this->getDb()->escapeString($this->getSearchString()).'\') + MATCH ('
            .ReferenceModel::COLUMN_REFERENCE_LABEL.'_'.Locales::getLang()
            .') AGAINST (\''.$this->getDb()->escapeString($this->getSearchString()).'\')', Db::COLUMN_ALL))
      ->where('MATCH('.ReferenceModel::COLUMN_REFERENCE_NAME.'_'.Locales::getLang().')',
         "AGAINST ('".$this->getDb()->escapeString($this->getSearchString())."' IN BOOLEAN MODE)", null,Db::COND_OPERATOR_OR)
      ->where('MATCH('.ReferenceModel::COLUMN_REFERENCE_LABEL.'_'.Locales::getLang().')',
      ...
      while ($row = $this->getDb()->fetchAssoc($sqlSearch)) {
         $this->addResult($this->getCategory($row[ReferenceModel::COLUMN_REFERENCE_ID_ITEM]),
            ...
            $row[ReferenceModel::COLUMN_REFERENCE_LABEL.'_'.Locales::getLang()],
            $row[parent::RESULT_INDEX_RELEVANCE],
            $row[ReferenceModel::COLUMN_REFERENCE_NAME.'_'.Locales::getLang()]);
      ...
      ->colums(array(
            parent::RESULT_INDEX_RELEVANCE => 'MATCH ('.ReferenceModel::COLUMN_TEXT.'_'.Locales::getLang()
            .') AGAINST (\''.$this->getDb()->escapeString($this->getSearchString()).'\')', Db::COLUMN_ALL))
      ->join(array('refer' => $this->getModule()->getDbTable()),
         ...
         null, array(ReferenceModel::COLUMN_REFERENCE_NAME.'_'.Locales::getLang()))
      ->where('MATCH('.ReferenceModel::COLUMN_TEXT.'_'.Locales::getLang().')',
      ...
      while ($row = $this->getDb()->fetchAssoc($sqlSearch)) {
         $this->addResult($this->getCategory($row[ReferenceModel::COLUMN_REFERENCE_ID_ITEM]),
            $this->getLink($row[ReferenceModel::COLUMN_REFERENCE_ID_ITEM]),
            $row[ReferenceModel::COLUMN_TEXT.'_'.Locales::getLang()], $row[parent::RESULT_INDEX_RELEVANCE]
          ,$row[ReferenceModel::COLUMN_REFERENCE_NAME.'_'.Locales::getLang()]
            );
      }
	}
