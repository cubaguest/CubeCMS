...
class BandsProgram_Controller extends Controller {
   ...
   public function editController() {
      ...
      if($form->isValid()) {
         ...
         foreach ($types as $key => $type) {
            ...
            switch ($type) {
               case "band":
                  $xml->writeElement('time', $times[$key]);
                  $xml->writeElement('bandid', $bandids[$key]);
                  break;
               case "day":
               case "stage":
               case "other":
                  $xml->writeElement('time', $times[$key]);
                  $xml->writeElement('text', $texts[$key]);
                  break;
               case "note":
                  $xml->writeElement('text', $textslong[$key]);
                  break;
               default:
                  break;
            }
   ...
   public function programItemController() {
      ...
      if($this->view()->type == "band" OR $this->view()->type == "other") {
         ...
         $this->view()->form->time->setValues('12:00');
      }
