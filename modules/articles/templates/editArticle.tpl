...
<form method="post" action="{$THIS_PAGE_LINK}">
	...
	<p id="articleLang{$smarty.foreach.lang.iteration}" class="tabcontent">
		...
		<textarea rows="15" cols="60" name="article_text[{$KEYLANG}]" class="textarea{if $VARS.ERROR_ITEMS.text.$KEYLANG eq true} badItem{/if}">{$VARS.ARTICLE_DATA.text.$KEYLANG}</textarea>
	</p>
	{/foreach}
	<br />
