...
class MailsAddressBook_Controller extends Controller {
   ...
   public function addressListController()
   {
      ...
      if ($jqGrid->request()->isSearch()) {
         ...
         switch ($jqGrid->request()->searchType()) {
            case Component_JqGrid_Request::SEARCH_EQUAL:
               $modelAddresBook->where($jqGrid->request()->searchField() . ' = :str', array('str' => $jqGrid->request()->searchString()));
               break;
            case Component_JqGrid_Request::SEARCH_NOT_EQUAL:
               $modelAddresBook->where($jqGrid->request()->searchField() . ' != :str', array('str' => $jqGrid->request()->searchString()));
               break;
            case Component_JqGrid_Request::SEARCH_NOT_CONTAIN:
               $modelAddresBook->where($jqGrid->request()->searchField() . ' NOT LIKE :str', array('str' => '%' . $jqGrid->request()->searchString() . '%'));
               break;
            case Component_JqGrid_Request::SEARCH_CONTAIN:
            default:
               $modelAddresBook->where($jqGrid->request()->searchField() . ' LIKE :str', array('str' => '%' . $jqGrid->request()->searchString() . '%'));
               break;
         }
      } else {
         // list
      ...
      if ($idGrp != 0) {
         $modelAddresBook->where(($where ? ' AND ' : '' ) . MailsAddressBook_Model_Addressbook::COLUMN_ID_GRP . ' = :idg', array('idg' => $idGrp), true);
      }
   ...
   public function groupsListController()
   {
      ...
      if ($jqGrid->request()->isSearch()) {
         switch ($jqGrid->request()->searchType()) {
            case Component_JqGrid_Request::SEARCH_EQUAL:
               $model->where($jqGrid->request()->searchField() . ' = :str', array('str' => $jqGrid->request()->searchString()));
               break;
            case Component_JqGrid_Request::SEARCH_NOT_EQUAL:
               $model->where($jqGrid->request()->searchField() . ' != :str', array('str' => $jqGrid->request()->searchString()));
               break;
            case Component_JqGrid_Request::SEARCH_NOT_CONTAIN:
               $model->where($jqGrid->request()->searchField() . ' NOT LIKE :str', array('str' => '%' . $jqGrid->request()->searchString() . '%'));
               break;
            case Component_JqGrid_Request::SEARCH_CONTAIN:
            default:
               $model->where($jqGrid->request()->searchField() . ' LIKE :str', array('str' => '%' . $jqGrid->request()->searchString() . '%'));
               break;
         }
      }
   ...
   public function editMailController()
   {
      ...
      switch ($jqGridReq->getRequest()) {
         ...
         case Component_JqGrid_FormRequest::REQUEST_TYPE_ADD:
            ...
            $record->{MailsAddressBook_Model_Addressbook::COLUMN_NAME} = $jqGridReq->{MailsAddressBook_Model_Addressbook::COLUMN_NAME};
            $record->{MailsAddressBook_Model_Addressbook::COLUMN_SURNAME} = $jqGridReq->{MailsAddressBook_Model_Addressbook::COLUMN_SURNAME};
            $record->{MailsAddressBook_Model_Addressbook::COLUMN_MAIL} = $jqGridReq->{MailsAddressBook_Model_Addressbook::COLUMN_MAIL};
            $record->{MailsAddressBook_Model_Addressbook::COLUMN_NOTE} = $jqGridReq->{MailsAddressBook_Model_Addressbook::COLUMN_NOTE};
            // id skupiny se vrací jako název, v tabulce je zobrazeno jako název ale z formu leze id   
            $record->{MailsAddressBook_Model_Addressbook::COLUMN_ID_GRP} = $jqGridReq->{MailsAddressBook_Model_Addressbook::COLUMN_ID_GRP};
   ...
   public function sendMailsListController()
   {
      ...
      $compScroll->setConfig(Component_Scroll::CONFIG_CNT_ALL_RECORDS, $model->getCount());
      $compScroll->setConfig(Component_Scroll::CONFIG_RECORDS_ON_PAGE, self::NUM_MAIL_IN_LIST);
      ...
      if ($formImportUsers->isValid()) {
         ...
         foreach ($users as $u) {
            Mails_Model_Addressbook::addUniqueMail(
                    $u->{Model_Users::COLUMN_MAIL}, $formImportUsers->group->getValues(), $u->{Model_Users::COLUMN_NAME}, $u->{Model_Users::COLUMN_SURNAME});
         }
...
//               $newExcelWriter->save(AppCore::getAppCacheDir()."mail_export.xls");
      ...
      $eAllGroups = new Form_Element_Checkbox('groups', $this->tr('Omezit duplikace na skupiny'));
      $eAllGroups->setValues(true);
      $formRemoveDuplicity->addElement($eAllGroups);
      ...
      if ($formRemoveDuplicity->isValid()) {
         ...
         if ($formRemoveDuplicity->groups->getValues() == true) {
            $groups = MailsAddressBook_Model_Groups::getAllRecords();

            foreach ($groups as $grp) {
//               $duplications = $modelDup
//                       ->columns(array(Mails_Model_Addressbook::COLUMN_MAIL))
//                       ->where(MailsAddressBook_Model_Addressbook::COLUMN_ID_GRP . ' = :idg', array('idg' => $grp->getPK()))
//                       ->groupBy(array(Mails_Model_Addressbook::COLUMN_MAIL))
//                       ->having('COUNT(*) > 1')
//                       ->records();
//               var_dump($modelDup->getSQLQuery());
               
               $duplications = $modelDup->getDuplicatedMials($grp->getPK());
...
//                  $modelDup->delete($dup);
//                  var_dump($dup->{MailsAddressBook_Model_Addressbook::COLUMN_MAIL});
               }
//               var_dump($grp->{MailsAddressBook_Model_Groups::COLUMN_NAME});
            }
